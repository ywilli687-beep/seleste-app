import type {
  PillarMeta, PillarId, PillarScores, PageSignals, Rule,
  AppliedRule, Vertical, RevenueLeak, AuditConfidence,
  Recommendation, RecommendationSet, VerticalBenchmark,
} from '@/types/audit'
...
export const PILLAR_DB_FIELD: Record<PillarId, string> = {
...
export const RULES: Record<PillarId, Rule[]> = {
  conversion: [
    ...
    { id: 'high_friction', label: 'High conversion friction detected', cap: 0, pen: 12, fn: s => (s.conversionFriction ?? 0) >= 4 },
...
export function applyRules(signals: PageSignals): {
  caps: Partial<Record<PillarId, number>>
  penalties: Partial<Record<PillarId, number>>
  applied: AppliedRule[]
} {
  const caps: Partial<Record<PillarId, number>> = {}
  const penalties: Partial<Record<PillarId, number>> = {}
  const applied: AppliedRule[] = []

  for (const [pillarId, rules] of Object.entries(RULES) as [PillarId, Rule[]][]) {
    let cap = 100
    let pen = 0

    for (const rule of rules) {
      if (rule.fn(signals)) {
        const base = Math.min(100, baseScorers[pillarId]?.(signals) ?? 50)

        // Apply this rule's contribution before recording
        if (rule.cap > 0 && rule.cap < cap) cap = rule.cap
        if (rule.pen > 0) pen += rule.pen
    ...
    if (cap < 100) caps[pillarId] = cap
    if (pen > 0) penalties[pillarId] = pen
  }
...
const baseScorers: Record<PillarId, (s: PageSignals) => number> = {
  conversion: s => {
    let n = 10
    if (s.hasCTA) n += 22; if (s.hasBooking) n += 25; if (s.hasContactForm) n += 13
    if (s.hasPricing) n += 10; if (s.hasGuarantee) n += 7; if (s.hasLiveChat) n += 8
    if (s.hasOnlinePayment) n += 5
    return n
  },
  trust: s => {
    let n = 5
    if (s.hasSSL) n += 15; if (s.hasContactInfo) n += 12; if (s.hasAddress) n += 10
    if (s.hasReviews) n += 22; if (s.hasAboutPage) n += 10; if (s.hasTeamPhotos) n += 8
    if (s.hasLicenseInfo) n += 7; if (s.hasAwardsOrCerts) n += 6; if (s.hasInsuranceInfo) n += 5
    return n
  },
  performance: s => {
    let n = 35
    if (s.isMobileOptimized) n += 22; if (s.hasOptimizedImages) n += 13
    if (s.hasLazyLoad) n += 10; if (s.hasCDN) n += 12; if (s.hasCaching) n += 8
    return Math.min(n, 95)
  },
  ux: s => {
    let n = 25
    if (s.hasGoodNav) n += 22; if (s.hasMobileMenu) n += 18; if (s.isMobileOptimized) n += 15
    if (s.hasServiceList) n += 8; if (s.hasSiteSearch) n += 7; if (s.hasAccessibility) n += 5
    return n
  },
  discoverability: s => {
    let n = 10
    if (s.hasGBP) n += 25; if (s.hasSchema) n += 18; if (s.hasMetaDescription) n += 13
    if (s.hasH1) n += 10; if (s.hasBlog) n += 12; if (s.hasLocalKeywords) n += 12
    return n
  },
  content: s => {
    let n = 10
    if (s.wordCount >= 800) n += 25; else if (s.wordCount >= 400) n += 18; else if (s.wordCount >= 300) n += 12
    if (s.hasServiceList) n += 18; if (s.hasFAQ) n += 13; if (s.hasBlog) n += 10
    if (s.hasLocationPages) n += 8; if (s.hasVideoContent) n += 8; if (s.hasBeforeAfterPhotos) n += 8
    return n
  },
  data: s => {
    let n = 5
    if (s.hasAnalytics) n += 35; if (s.hasPixel) n += 25; if (s.hasTagManager) n += 18
    if (s.hasHeatmaps) n += 10; if (s.hasCRMIntegration) n += 7
    return n
  },
  technical: s => {
    let n = 35
    if (s.hasSSL) n += 18; if (s.hasCMS) n += 15; if (s.hasXMLSitemap) n += 12
    if (s.hasStructuredData) n += 12; if (s.techStack.length > 0) n += 8
    return Math.min(n, 95)
  },
  brand: s => {
    let n = 20
    if (s.hasLogo) n += 30; if (s.hasBrandDiff) n += 25; if (s.hasTagline) n += 15
    if (s.hasConsistentColors) n += 10
    return n
  },
  scalability: s => {
    let n = 20
    if (s.hasCMS) n += 30; if (s.hasAnalytics) n += 18; if (s.hasPixel) n += 12
    if (s.hasCRMIntegration) n += 15; if (s.hasTagManager) n += 5
    return n
  },
}
...
export function computePillarScores(
  ...
  for (const p of PILLARS) {
    ...
    score = Math.min(score, caps[p.id] ?? 100)
    score = Math.max(0, score - (penalties[p.id] ?? 0))
    scores[p.id] = Math.round(Math.min(100, Math.max(0, score)))
  }
  return scores
}

export function computeWeightedScore(scores: PillarScores): number {
  return Math.round(PILLARS.reduce((acc, p) => acc + scores[p.id] * p.weight, 0))
}

export function computeGrade(score: number): { grade: 'A' | 'B' | 'C' | 'D'; label: string } {
...
export function computeConfidence(signals: PageSignals): AuditConfidence {
  const missing: string[] = []
  let deductions = 0

  // ── Data quality checks — things that reduce audit reliability ────────────

  // Word count too low to draw meaningful content conclusions
  if (signals.wordCount < 150) {
    deductions += 20
    missing.push('Very little page content detected — page may be JS-rendered or gated')
  }

  // No page title extracted — likely a fetch or parse failure
  if (!signals.pageTitle) {
    deductions += 15
    missing.push('Page title not extracted — HTML may be incomplete')
  }

  // No H1 AND no meta description — suggests the page didn't load properly
  if (!signals.hasH1 && !signals.hasMetaDescription) {
    deductions += 15
    missing.push('No H1 or meta description — page structure may not have loaded correctly')
  }

  // AI returned no services or page summary — soft signal extraction may have failed
  if (signals.primaryServices.length === 0 && !signals.pageSummary) {
    deductions += 15
    missing.push('Could not identify business type or services — AI extraction may be incomplete')
  }

  // No contact info of any kind — could indicate wrong page fetched
  if (!signals.hasContactInfo && !signals.hasAddress && !signals.pageTitle) {
    deductions += 10
    missing.push('No contact information found — may have fetched the wrong page')
  }

  // Review count unverifiable without GBP API (honest gap, small deduction)
  if (signals.estimatedReviewCount == null) {
    deductions += 5
    missing.push('Review count unverified — estimated from page only')
  }

  // GBP signal relies on page content — may be present but not detectable
  if (!signals.hasGBP) {
    deductions += 5
    missing.push('GBP link not detected on page — may exist but not be embedded')
  }

  const pct = Math.max(10, 100 - deductions)
...
export const STATIC_BENCHMARKS: Record<Vertical, VerticalBenchmark> = {
...
// ── Vertical groups ────────────────────────────────────────────────────────────
// Maps each Vertical to one of 4 recommendation copy pools so advice is
// actually relevant to the business type. A car wash and a law firm both
// need a booking system — but the framing, platform suggestions, and urgency
// language should be completely different.

type VerticalGroup = 'food_retail' | 'trades_home' | 'health_legal' | 'beauty_fitness'

const VERTICAL_GROUP: Record<string, VerticalGroup> = {
...
const RECS_BY_GROUP: Record<VerticalGroup, Recommendation[]> = {
  food_retail: [
...
export function selectRecommendations(scores: PillarScores, vertical?: string): RecommendationSet {
  const weak = [...PILLARS].sort((a, b) => scores[a.id] - scores[b.id]).slice(0, 6).map(p => p.id)
  const group: VerticalGroup = (vertical && VERTICAL_GROUP[vertical]) ? VERTICAL_GROUP[vertical] : 'trades_home'
...
export function buildRoadmap(
  ...
  const sprint30: string[] = []
  if (fired.has('no_gbp')) sprint30.push('Claim / optimize Google Business Profile')
  if (fired.has('no_ssl')) sprint30.push('Install SSL certificate + redirect HTTP → HTTPS')
  if (fired.has('no_cta')) sprint30.push('Add primary CTA button above the fold')
  if (fired.has('no_reviews')) sprint30.push('Email last 20 customers requesting a Google review')
  if (fired.has('no_analytics')) sprint30.push('Install Google Analytics 4 with conversion tracking')
  if (fired.has('no_form')) sprint30.push('Add contact form with confirmation page tracking')
  if (fired.has('no_address')) sprint30.push('Add physical address to header and footer')
  if (fired.has('no_contact')) sprint30.push('Make phone number click-to-call on mobile')

  const sprint60: string[] = []
  if (fired.has('no_booking')) sprint60.push('Integrate online booking (Calendly, Setmore, or custom)')
  if (fired.has('no_pixel')) sprint60.push('Install Meta Pixel and build first retargeting audience')
  if (fired.has('no_schema')) sprint60.push('Add LocalBusiness + FAQ schema markup')
  if (fired.has('no_img_opt')) sprint60.push('Convert all images to WebP and enable lazy loading')
  if (fired.has('no_team')) sprint60.push('Add team / about page with real staff photos and bios')
  if (fired.has('no_services')) sprint60.push('Build a clear, specific service list with descriptions')
  if (signals.wordCount < 600) sprint60.push('Expand homepage content to 600+ words with local keywords')
  if (fired.has('no_local_kw')) sprint60.push('Write 4 location-specific service landing pages')

  const sprint90: string[] = []
  if (fired.has('no_blog') || scores.content < 50)
    sprint90.push('Launch monthly local content program (4 articles/mo)')
  if (scores.trust < 55) sprint90.push('Run active Google review collection via email + SMS')
  ...
  while (sprint30.length < 4 && pad30.length) sprint30.push(pad30.shift()!)
  while (sprint60.length < 4 && pad60.length) sprint60.push(pad60.shift()!)
