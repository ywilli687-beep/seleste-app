...
export function computeRevenueLeak(scores: PillarScores, monthlyRevenue?: number): RevenueLeak {
  ...
  return {
    ...
    breakdown: [
      { label: 'Conversion gaps',      pillar: 'conversion'  as PillarId, pct: Math.round(cl * 100), icon: '🎯' },
      { label: 'Trust deficit',        pillar: 'trust'       as PillarId, pct: Math.round(tl * 100), icon: '🛡️' },
      { label: 'Performance friction', pillar: 'performance' as PillarId, pct: Math.round(pl * 100), icon: '⚡' },
      { label: 'UX friction',          pillar: 'ux'          as PillarId, pct: Math.round(ul * 100), icon: '✨' },
    ].filter(x => x.pct > 0),
  }
}
...
export function computeConfidence(signals: PageSignals): AuditConfidence {
  ...
  if (!signals.hasGBP)      missing.push('Google Business Profile not linked — local SEO data limited')
  if (!signals.hasAnalytics) missing.push('No analytics platform detected')
  if (!signals.hasSchema)    missing.push('No schema markup — rich results unavailable')
  if (signals.estimatedReviewCount == null) missing.push('Review count unverified — requires GBP API')
