...
export function AgentsShell({ data: initialData }: { data: AgentsPageData }) {
  ...
  const handleRunManual = () => {
    // triggers POST /api/agents/weekly 
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: 24 }}>
             ...
             <div className="card-v2" style={{ padding: 24 }}>
                <h3 className="text-h2" style={{ marginBottom: 16 }}>This Week's Actions ({data.weeklyActions?.length || 0})</h3>
                <span className="text-small text-body" style={{ color: 'var(--ink-muted)' }}>From Growth Architect</span>
                <div style={{ marginTop: 16 }}>
                   {data.weeklyActions?.length === 0 ? (
                     <p className="text-body" style={{ fontSize: 13 }}>No actions generated yet.</p>
                   ) : (
                     <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                       {data.weeklyActions.map((action, i) => (
                         <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 12, border: '1px solid var(--border)', borderRadius: 8 }}>
                           <input type="checkbox" />
             ...
             {/* REPORTING SUMMARY */}
             <div className="card-v2" style={{ padding: 24, background: 'var(--panel)' }}>
                <h3 className="text-h2" style={{ marginBottom: 16 }}>Executive Summary</h3>
                <span className="text-small text-body" style={{ color: 'var(--ink-muted)' }}>From Reporting Agent</span>
                <p className="text-body" style={{ marginTop: 16, fontSize: 13, lineHeight: 1.6 }}>
                  {data.reportingOutput?.executiveSummary || "Summary data is compiling..."}
                </p>
             </div>
          </div>
          ...
          {/* AGENT STATUS GRID */}
          <div className="card-v2" style={{ padding: 24 }}>
            <h3 className="text-h2" style={{ marginBottom: 24 }}>Agent Execution Grid</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
              {agentOutputs.map(ag => (
                <div key={ag.agentId} style={{ 
                  ...
                  {ag.status === 'failed' && <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 4, background: 'var(--coral)' }} />}
                  {ag.status === 'skipped' && <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 4, background: 'rgba(255,255,255,0.1)' }} />}
                  
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ width: 32, height: 32, borderRadius: 8, background: `var(--${ag.chipColor})`, opacity: 0.2 }} />
                    <span style={{ fontWeight: 600, fontSize: 13 }}>{ag.agentName}</span>
                  </div>
                  
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
                    <span className="chip" style={{ 
                      fontSize: 11, background: 'transparent', padding: 0, 
                      color: ag.status === 'failed' ? 'var(--coral)' : ag.status === 'skipped' ? 'var(--ink-muted)' : 'var(--green2)' 
                    }}>
                      {ag.status === 'pending' ? 'Pending' : ag.status === 'complete' ? 'Complete' : ag.status === 'skipped' ? 'Skipped' : 'Failed'}
                    </span>
                    <span style={{ fontSize: 11, color: 'var(--ink-muted)' }}>{(ag.durationMs||0)/1000}s</span>
                  </div>
                </div>
              ))}
            </div>
                      ...
                      <div key={ig.platform} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 16, border: '1px solid var(--border)', borderRadius: 8, background: 'var(--page-bg)' }}>
                         <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                           ...
                           <div style={{ display: 'flex', flexDirection: 'column' }}>
                             <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--ink)' }}>{ig.displayName}</span>
