        ...
        <header style={{ padding: '40px 0 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          ...
          {isStale && (
            <div className="chip chip-amber" style={{ padding: '8px 16px', fontSize: 13, cursor: 'pointer' }} onClick={handleAuditNavigation}>
              Data is {data?.daysSinceAudit ?? 0} days old. Run a new audit.
            </div>
          )}
        </header>
            ...
            <>
                  ...
                  <div style={{ padding: 32, background: 'var(--bg2)', borderRadius: 'var(--r)', border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', justifyContent: 'center', height: '100%' }}>
                    ...
                    <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text2)' }}>
                        {(data?.overallScore ?? 0) >= 75 ? 'Great' : 
                         (data?.overallScore ?? 0) >= 60 ? 'Good' : 
                         (data?.overallScore ?? 0) >= 45 ? 'Needs Work' : 'Critical Issues'}
                      </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: 24 }}>
                <StreakCard 
                  ...
                  streakPtsThisMonth={data?.streakPtsThisMonth ?? 0}
              ...
              <div>
                ...
                <MarketStrip 
                  verticalMedianScore={data?.verticalMedianScore ?? 75}
                  bookingAdoptionRate={data?.bookingAdoptionRate ?? 50}
                  topGap={data?.topGap ?? 'N/A'}
                  avgMonthlyImprovement={data?.avgMonthlyImprovement ?? 0}
                />

                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginTop: 24 }}>
                  <CompetitorCard 
                    userScore={data?.overallScore ?? 0}
                    competitors={data?.competitorScores ?? []}
                    gap={data?.competitorGap ?? 0}
