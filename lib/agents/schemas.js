...
export const auditSchema = z.object({
    qualityScore: z.number().int().min(0).max(100).describe('Most local businesses score 25-55; be honest'),
    ...
    findings: z.array(z.object({
        ...
        action: z.string(),
        impactScore: z.number().int().min(1).max(10).describe('1-10 score of revenue impact'),
        difficultyLevel: z.number().int().min(1).max(10).describe('1-10 score of effort needed'),
        estRevenueImpact: z.string().describe('Estimate like "$1k - 3k MRR" or "$500/mo"')
    })).length(5, 'Return exactly 5 findings ranked by impact'),
    ...
    })).length(6, 'Return exactly 6 roadmap tasks'),
    competitors: z.array(z.object({
        name: z.string(),
        domain: z.string(),
        estimatedTraffic: z.number().int(),
        marketShare: z.number().describe('0.0 to 1.0 estimate'),
        adPresence: z.boolean()
    })).length(3, 'Identify top 3 probable competitors in this local market').optional()
});

// The structure for generating 90-day execution plans
export const marketingPlanSchema = z.object({
    executiveSummary: z.array(z.string()).length(5, 'Exactly 5 bullet points'),
    ...
    })).length(3, 'Must contain arrays for 30, 60, and 90 day phases'),
